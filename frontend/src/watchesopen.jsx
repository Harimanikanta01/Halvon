...
function Watchesopen() {
 ...
 const backend=(process.env.REACT_APP_URL).replace(/\/+$/, "");
  const Ab = async () => {
    try {
      const res = await axios.get(`${backend}/watches/${id}`);
      setData(res.data);
    } catch (error) {
      console.log(error);
    }
...
const Hoe=async(name,price,img1)=>{
  const name1=localStorage.getItem("name1")
  const pp={name,price,img1,name1}
  const Tpo=await axios.post(`${backendurl}/cartpost`,pp)
  console.log(Tpo.data)
}
  return (
    <>
      <div
      ...
      >
        ...
        <div style={{ flex: "1", padding: "20px" }}>
          ...
          <div style={{ display: "flex", gap: "20px" }}>
            ...
            <button onClick={()=>Hoe(data.name,data.price,data.img1)}
              style={{
                backgroundColor: "#333",
                color: "white",
                border: "none",
